...
const vertex = /* glsl */ `
  ...
  void main() {
    ...
    // Repulsion logic
    if (uHover > 0.5) {
      vec2 mousePosWorld = uMouse * 15.0;
      float dist = length(mPos.xy - mousePosWorld);
      if (dist < 6.0) {
        vec2 dir = normalize(mPos.xy - mousePosWorld);
        float force = (6.0 - dist) * 1.2;
        mPos.xy += dir * force;
      }
    }
  ...
  useEffect(() => {
    ...
    const update = t => {
      ...
      if (moveParticlesOnHover) {
        ...
        program.uniforms.uHover.value = 1;
